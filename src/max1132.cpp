...
6=1, unipolar
5=0, internal clock
4,3 = 11, 32 External clocks per conversion, long aq mode
2,1,0=0, unused
...
uint16_t MAX1132::read()
{
    ...
    SPI.transfer(m_readbyte_long);
    SPI.transfer(m_transfer_byte);                 // discard first byte
    reading |= SPI.transfer(m_transfer_byte) << 8; // read the first 8 bits
    reading |= SPI.transfer(m_transfer_byte);
